...
int A = 0;
int B = 0;
int C = 0; //COOLER READING 21'C @ 266
int H = 0; //HOUR
int M = 0; //MIN
int S = 0; //SECONDS
int screen = 0; //SCREEN COUNT 7
...
void loop(){
  ...
  if(CS == 1){
	...
	if(screen == 3){												//screen 3(4)
		if(irCode == 0x371A3C86){OL = !OL;}
	}
	if(screen == 1){												//screen 1
		if(irCode == 0x143226DB){if(ML < 11){ML++;}else{ML = ML + 10;}if(ML > 255){ML = 255;}}
		if(irCode == 0xA32AB931){if(ML < 9){ML--;}else{ML = ML - 10;}if(ML < 0){ML = 0;}}
		if(irCode == 0x371A3C86){if(BL < 11){BL++;}else{BL = BL + 10;}if(BL > 255){BL = 255;}}
		if(irCode == 0x39D41DC6){if(BL < 9){BL--;}else{BL = BL - 10;}if(BL < 0){BL = 0;}}
	}
	MAX = MIN + DEF;
	if(CS == 0){CF = 0;}
	analogWrite(3, ML);
	analogWrite(5, BL);
	if(OL == 1){analogWrite(6, 200);}else{analogWrite(6, 0);}
	digitalWrite(2, SL);
	digitalWrite(4, CF);
...
void lcdDisplay(){
	...
	if(screen == 3){
		...
		lcd.setCursor(0, 0);
		lcd.print("OUT LIGHT:");
    if(OL == 0){lcd.print("OFF");}else{lcd.print("ON");}
		lcd.setCursor(0, 1);
    lcd.print("COOLER: ");
